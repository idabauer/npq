...
#	day.of.computations	<- "2022-09-19"	
...
for(cf in config.set){
  ...
  res		<- data.frame(array(data = NA, dim = c(S, 3), dimnames = list(NULL, c("h", "ISV", "ISVC"))))
  res$h	<- 1:S
...
matplot(res, xaxt = "n", type = "n", ylim = c(45, 95), frame.plot = FALSE, xlab = "h", ylab = paste("Coverage (nominal: ", nomin, "%)", sep = ""))

axis(side = 1, at = res$h, labels = res$h)
abline(h = nomin, col = 1, lty = 2)
matlines(res[, -1], type = "b", pch = 19, col = col.set, lty = 1)
text(x = min(res$h), y = res[1, -1] + c(-2, +2), labels = c("IS", expression(IS[c])), col = col.set)
